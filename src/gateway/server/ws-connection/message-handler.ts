  ...
  socket.on("message", async (data) => {
    ...
    try {
      ...
      if (!client) {
        ...
        const allowGatewayClientScopesWithoutDevice = parseBooleanEnv(
          process.env.OPENCLAW_ALLOW_GATEWAY_CLIENT_SCOPES_WITHOUT_DEVICE,
        );
        const clearUnboundScopes = () => {
          const allowBypassForGatewayClient =
            allowGatewayClientScopesWithoutDevice &&
            sharedAuthOk &&
            connectParams.client.id === GATEWAY_CLIENT_IDS.GATEWAY_CLIENT;
          if (
            scopes.length > 0 &&
            !controlUiAuthPolicy.allowBypass &&
            !allowBypassForGatewayClient
          ) {
            scopes = [];
            connectParams.scopes = scopes;
          }
